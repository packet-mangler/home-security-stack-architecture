...
| Secrets management | Infisical | Container secret storage |
...
- **Docker host migration**: Interim containers (Portainer, opnsense-mcp, Infisical) running on NAS pending migration to a dedicated Docker VM on Proxmox.
- **Cloud-init templates**: All Linux VMs provisioned from a single Ubuntu 24.04 cloud-init template (VMID 9000). ISO installs reserved for Kali and Windows only.
---
...
- Docker host migration to Proxmox
